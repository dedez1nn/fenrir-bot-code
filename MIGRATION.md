...
| 3 — Loja + cooldowns no DB | Pendente |
| 4 — Usuários + XP/Coins no DB | Pendente |
...
- [ ] `cogs/economia/loja.py` → queries em `items` (CRUD)
- [ ] `cogs/economia/cooldown.py` → queries em `cooldowns` com `expires_at` indexado
- [ ] `cogs/economia/comands_loja.py` (admin add/edit/remove item) → mesmas queries
- [ ] Criar camada `repositories/` reutilizável: `repositories/items.py`, `repositories/cooldowns.py`
- [ ] API: `GET /items`, `POST /items`, `PATCH /items/{id}`, `DELETE /items/{id}`
- [ ] `import_legacy_json()` já importa `loja_data.json` e `cooldowns_data.json` automaticamente quando as tabelas estão vazias

### Phase 4 — Usuários + XP/Coins no DB

- [ ] `cogs/economia/fenrir_coins.py` e `cogs/progressao/xp.py` → queries em `users`
- [ ] Race condition resolvida com `UPDATE users SET coins = coins + $1 WHERE user_id = $2 RETURNING coins` atômico
- [ ] `voice_xp_loop` e `dobro_xp_loop` com batch updates via `executemany`
- [ ] Eliminar `self.user_data` em memória (cache passa a ser por-comando)
- [ ] `repositories/users.py` com métodos: `get`, `add_coins`, `add_xp`, `set_premium`, etc.
- [ ] API: `GET /users`, `GET /users/{id}`, `PATCH /users/{id}/premium`
...
- [ ] `api/routers/webhooks.py`: validação HMAC do Mercado Pago, atualização de `users.premium` + `users.premium_expira`
- [ ] `api/routers/config.py` ganha PUT/PATCH com `bot.reload_config()`
- [ ] `api/routers/users.py`: busca, paginação, ativação manual de premium
- [ ] `api/routers/items.py`: CRUD da loja
